            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-gray-800">Lista de Usuarios Inactivos</h3>
                            <p class="text-sm text-gray-600 mt-1">Usuarios que han sido desactivados del sistema</p>
                        </div>
                                ...
                                @foreach($usuarios as $usuario)
                                    <tr class="bg-red-50 hover:bg-red-100 transition duration-150">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="font-semibold text-red-900">{{ $usuario->username }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="text-sm text-red-800">{{ $usuario->nombre }} {{ $usuario->apellido }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="text-sm text-red-800">{{ $usuario->ci }}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="text-sm text-red-800">{{ $usuario->correo }}</span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex items-center gap-3">
                                                <!-- Botón Reactivar -->
                                                <form action="{{ route('usuarios.reactivar', $usuario->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit" 
                                                ...
                                                <form action="{{ route('usuarios.forceDestroy', $usuario->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                                                            class="inline-flex items-center px-3 py-1.5 bg-red-700 hover:bg-red-800 text-white text-xs font-medium rounded transition"
                                                            onclick="return confirm('⚠️ ADVERTENCIA: Esto eliminará el usuario PERMANENTEMENTE de la base de datos.\n\nNo se podrá recuperar.\n\n¿Estás COMPLETAMENTE seguro?')">
                                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @else
                    <div class="p-12 text-center">
                        ...
                        <h3 class="text-lg font-semibold text-gray-800 mb-2">¡No hay usuarios inactivos!</h3>
                        <p class="text-gray-600">Todos los usuarios están activos en el sistema</p>
                    </div>
                @endif
            </div>
        </div>
