                            ...
                            @forelse($docentes as $docente)
                                <tr class="hover:bg-gray-50 transition duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="font-semibold text-gray-800">{{ $docente->registro }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-600">{{ $docente->usuario->nombre }} {{ $docente->usuario->apellido }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-600">{{ $docente->usuario->ci }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-600">{{ $docente->carrera ?? 'N/A' }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm text-gray-600">{{ $docente->especialidad ?? 'N/A' }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <form action="{{ route('docentes.destroy', $docente->registro) }}" 
                                                  method="POST" 
                                                  class="inline"
                                                  onsubmit="return confirm('¿Estás seguro de desactivar este docente?');">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center">
                                        ...
                                        <p class="mt-2 text-gray-500">No hay docentes activos registrados</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
